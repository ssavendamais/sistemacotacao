...
import { AlertCircle, ArrowLeft, Building2, CheckCircle2, Clock, FileText, Info, Package } from "lucide-react";
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-sm text-neutral-500 flex items-center gap-2">
              ...
              Empresa: <span className="font-semibold text-neutral-700">{(cotacao as any).profiles?.empresa || (cotacao as any).profiles?.nome}</span>
            </p>
          </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card className="bg-neutral-50/50 border-none">
            ...
            <CardBody className="space-y-4">
              <div className="space-y-1">
                <p className="text-[10px] text-neutral-400 font-bold uppercase">Descrição / Observações</p>
              ...
              <div className="pt-4 border-t border-neutral-100 flex items-center justify-between">
                <div className="text-center">
                  <p className="text-lg font-bold text-neutral-900">{cotacao.cotacao_itens?.length || 0}</p>
                  <p className="text-[10px] text-neutral-400 font-bold uppercase">Itens</p>
                </div>
                <div className="h-8 w-px bg-neutral-200" />
                <div className="text-center">
                  <p className="text-lg font-bold text-neutral-900">{formatDate(cotacao.created_at)}</p>
                ...
                {cotacao.cotacao_itens?.map((item: any) => (
                  <div key={item.id} className="p-4 flex justify-between items-start gap-3">
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-neutral-800 truncate">{item.nome_produto}</p>
                      {item.observacao && (
                        <p className="text-[10px] text-neutral-400 mt-0.5 line-clamp-1 italic">{item.observacao}</p>
                      )}
                    </div>
          ...
          <div className="bg-primary-50 p-4 rounded-[var(--radius-lg)] border border-primary-100 flex gap-3">
            ...
            <p className="text-xs text-primary-700 leading-relaxed">
              Dica: O empresário valoriza prazos de entrega curtos e observações detalhadas sobre a qualidade dos produtos.
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          {existingProposta ? (
            <div className="space-y-6 animate-fade-in">
                ...
                <CardBody className="p-8 flex flex-col items-center text-center gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-neutral-500 max-w-sm">
                      Você já enviou um orçamento para esta cotação. Agora basta aguardar o retorno do empresário.
                    </p>
                  </div>
              ...
              <div className="p-6 bg-neutral-50 rounded-[var(--radius-lg)] border border-neutral-100 flex items-start gap-4">
                ...
                <p className="text-xs text-neutral-500 leading-relaxed italic">
                  Nota: Se precisar alterar valores ou condições, você pode enviar uma nova proposta que substituirá a anterior após validação do empresário.
                </p>
              </div>
            </div>
          ) : (
            <EnviarPropostaForm
              ...
              cotacaoItens={cotacao.cotacao_itens ?? []}
            />
          )}
        </div>
      </div>
