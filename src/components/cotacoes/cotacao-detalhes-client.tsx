...
import { COTACAO_STATUS_LABELS, PROPOSTA_STATUS_LABELS } from "@/lib/constants";
import { formatCurrency, formatDate } from "@/lib/utils";
import { AlertCircle, ArrowLeft, CheckCircle2, Clock, Package, Trophy } from "lucide-react";
    ...
    <div className="space-y-6 animate-fade-in pb-12">
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-sm text-neutral-500">
              Criada em {formatDate(cotacao.created_at)} • {cotacao.propostas?.length || 0} propostas recebidas
            </p>
          </div>
          
          <StatusCotacaoButtons
            cotacaoId={cotacao.id}
            currentStatus={cotacao.status}
          />
        </div>
      </div>
      ...
      {activeTab === "propostas" && (
        <div className="space-y-6 animate-fade-in">
                ...
                <CardBody className="p-5 flex flex-col gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <Badge variant={p.status === "aceita" ? "success" : p.status === "recusada" ? "danger" : "warning"}>
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-neutral-400 font-medium uppercase tracking-tight">Valor Total Ofertado</p>
                  </div>
                </CardBody>
              </Card>
            ))}
              ...
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="bg-neutral-50 border-b border-neutral-200">
                    ...
                    <th className="px-4 py-3 text-center text-xs font-semibold text-neutral-500 uppercase">
                      Qtd
                    </th>
                    {cotacao.propostas?.map((p: any) => (
                      <th key={p.id} className="px-4 py-3 text-center text-xs font-bold uppercase min-w-[160px] text-neutral-800 border-l border-neutral-100">
                        {p.profiles?.empresa || p.profiles?.nome}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody className="divide-y divide-neutral-100">
                    ...
                    <tr key={item.id} className="hover:bg-neutral-50/50 transition-colors">
                      <td className="px-4 py-4 sticky left-0 bg-white z-10 shadow-[2px_0_5px_rgba(0,0,0,0.02)]">
                        <div>
                          ...
                          <p className="text-[10px] text-neutral-400 italic mt-0.5">{item.observacao || "—"}</p>
                        </div>
                      </td>
                      <td className="px-4 py-4 text-center text-neutral-600 font-medium">
                        {item.quantidade} <span className="text-[10px] uppercase">{item.unidade}</span>
                      </td>
                      {cotacao.propostas?.map((p: any) => {
                        const pi = p.proposta_itens?.find((pi: any) => pi.cotacao_item_id === item.id);
                        ...
                        return (
                          <td key={p.id} className={`px-4 py-4 text-center border-l border-neutral-50 ${
                            isCheapest ? "bg-green-50/40" : ""
                          }`}>
                            {pi ? (
                              <div className="space-y-1">
                                <p className={`text-sm font-bold ${isCheapest ? "text-green-700" : "text-neutral-700"}`}>
                                  {formatCurrency(pi.preco_unitario)}
                                </p>
                                <p className="text-[10px] text-neutral-400">
                                  Sub: {formatCurrency(pi.preco_unitario * item.quantidade)}
                                </p>
                                {isCheapest && (
                  ...
                  <tr className="bg-neutral-900 text-white font-bold">
                    <td colSpan={2} className="px-4 py-4 text-right text-xs uppercase tracking-widest">Valor Total</td>
                    {cotacao.propostas?.map((p: any) => (
          ...
          {/* Individual Proposal Cards (Actions) */}
          <div className="space-y-4">
            <h3 className="text-lg font-bold text-neutral-900 mt-8 mb-4">Gerenciar Propostas</h3>
            {cotacao.propostas?.map((proposta: any) => (
              <Card key={proposta.id} className={proposta.status === "aceita" ? "border-green-300 bg-green-50/10" : ""}>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                    <div className="space-y-3 flex-1">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <Badge variant={proposta.status === "aceita" ? "success" : proposta.status === "recusada" ? "danger" : "warning"}>
                    ...
                    <div className="text-right flex flex-col items-end gap-3 w-full sm:w-auto">
                      <div>
                        ...
                        <p className="text-[10px] text-neutral-400 font-bold uppercase tracking-wider">Valor Final</p>
                      </div>
            ...
            <Card>
              ...
              <CardBody className="grid grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-neutral-800 mt-1">{formatDate(cotacao.created_at)}</p>
                </div>
                <div className="col-span-2">
                  <label className="text-xs text-neutral-400 font-bold uppercase">Observações da Cotação</label>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-neutral-50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-bold text-neutral-500 uppercase">Item</th>
                      <th className="px-6 py-3 text-center text-xs font-bold text-neutral-500 uppercase">Qtd</th>
                      <th className="px-6 py-3 text-left text-xs font-bold text-neutral-500 uppercase">Obs</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-neutral-100">
                    {cotacao.cotacao_itens?.map((item: any) => (
                      <tr key={item.id}>
                        <td className="px-6 py-4 font-semibold text-neutral-800">{item.nome_produto}</td>
                        <td className="px-6 py-4 text-center font-medium text-neutral-600">{item.quantidade} <span className="text-[10px]">{item.unidade}</span></td>
                        <td className="px-6 py-4 text-neutral-400 italic text-xs">{item.observacao || "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <CardBody className="p-6 space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold">{cotacao.data_limite ? formatDate(cotacao.data_limite) : "A definir"}</p>
                  </div>
                </div>
                <div className="pt-4 border-t border-white/10">
                  <p className="text-xs text-primary-200 leading-relaxed italic">
                    Fornecedores podem enviar propostas até o encerramento da cotação ou atingir a data limite.
                  </p>
                </div>
              </CardBody>
            </Card>
